...
import educationImg from '../assets/education-study.jpg';
...
function Education() {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('category', 'Education');
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-5xl mx-auto px-8">
        <div className="relative h-96 rounded-3xl overflow-hidden mb-12 shadow-2xl">
          <img src={educationImg} alt="Education" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-12">
            <h1 className="text-6xl font-bold text-white">Education Travel Solutions That Shape Your Future</h1>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-12 rounded-3xl shadow-xl space-y-8">
          ...
          <input name="specialization" placeholder="Specialization (e.g. Student Visa, Study Abroad)" onChange={handleChange} className="w-full p-5 border border-gray-300 rounded-2xl text-lg" />
          <textarea name="message" placeholder="Tell us about your education travel needs" rows="5" onChange={handleChange} className="w-full p-5 border border-gray-300 rounded-2xl text-lg" required />
