...
function Conference() {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('category', 'Conference');
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-5xl mx-auto px-8">
        <div className="relative h-96 rounded-3xl overflow-hidden mb-12 shadow-2xl">
          <img src={conferenceImg} alt="Conference" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-12">
            <h1 className="text-6xl font-bold text-white">Conference Travel Solutions</h1>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-12 rounded-3xl shadow-xl space-y-8">
          ...
          <input name="specialization" placeholder="Conference / Event Name" onChange={handleChange} className="w-full p-5 border border-gray-300 rounded-2xl text-lg" />
          <textarea name="message" placeholder="Tell us about your conference travel needs" rows="5" onChange={handleChange} className="w-full p-5 border border-gray-300 rounded-2xl text-lg" required />
