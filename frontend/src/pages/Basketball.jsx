...
function Basketball() {
  ...
  const [subSpec, setSubSpec] = useState('NBA');  // Dropdown
  ...
  const handleSaveDraft = () => {
    ...
    localStorage.setItem('basketballDraft', JSON.stringify(draft));
    setSavedDraft(true);
    alert('Draft saved!');
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5000/api/inquiries', { category: 'Basketball', details: `${name} (${email}): ${message} - Sub: ${subSpec}, Spec: ${specialization}` });
      ...
      const response = await axios.post('https://api.oxapay.com/v1/createInvoice', {
        ...
        amount: 50,
        currency: 'USDT',
        returnUrl: 'http://localhost:5173/sports/basketball',
        description: 'Basketball Application Fee'
      });
  ...
  return (
    ...
    >
      <div className="text-center mb-12">
        <h1 className="text-5xl md:text-6xl font-bold text-indigo-900 mb-4 drop-shadow-lg">Basketball Travel Solutions</h1>
        <p className="text-xl text-gray-700 max-w-3xl mx-auto mb-8">
          NBA games, Olympics bookings, player visas.
        </p>
        <img
          src="/assets/basketball-court.jpg"
          alt="Basketball game"
      ...
      <div className="grid md:grid-cols-2 gap-8 mt-12">
        <motion.div initial={{ y: 20 }} animate={{ y: 0 }} className="bg-white p-6 rounded-2xl shadow-xl hover:scale-105 hover:rotate-2 transition duration-300">
          <h3 className="text-xl font-semibold mb-2 text-indigo-900">Games</h3>
          <p className="text-gray-600">NBA finals travel.</p>
        </motion.div>
        <motion.div initial={{ y: 20 }} animate={{ y: 0 }} className="bg-white p-6 rounded-2xl shadow-xl hover:scale-105 hover:rotate-2 transition duration-300">
          <h3 className="text-xl font-semibold mb-2 text-indigo-900">Camps</h3>
          <p className="text-gray-600">Youth training visas.</p>
        </motion.div>
      </div>
    </motion.div>
  );
